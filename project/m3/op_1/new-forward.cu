...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    */
    ...
    for (int c = 0; c < Channel; c++) {
        if ((h < Height) && (w < Width)) tile[idx] = in_4d(b, c, h, w);
        else tile[idx] = 0.0;
        __syncthreads();
        ...
        if ((tx < TILE_WIDTH) && (ty < TILE_WIDTH))
            for (int p = 0; p < K; p++)
                for (int q = 0; q < K; q++)
                    acc += tile[(ty+p) * block_width + (tx+q)] * mask_4d(m, c, p, q);
        __syncthreads();
    }
    __syncthreads();

    if ((h < Height_out) && (w < Width_out) && (tx < TILE_WIDTH) && (ty < TILE_WIDTH)) out_4d(b, m, h, w) = acc;
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    size_t shared_mem = block_width * block_width * sizeof(float);

    conv_forward_kernel<<<DimGrid, DimBlock, shared_mem>>>(device_output, device_input, device_mask, Batch, Map_out, Channel, Height, Width, K);
